...
export async function fetchUserBalance() {
  ...
  try {
    const response = await fetch(
      "https://book2500.funzip.in/api/user-balance",
      {
        headers: {
...
export async function fetchUserProfile() {
  ...
  try {
    const response = await fetch(
      "https://book2500.funzip.in/api/profile-setting",
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
...
async function validatePredictionData(
  data: PredictionData,
  ...
  try {
    ...
    const question = match.bet_questions.find(
      (q) => q.id === data.betquestion_id,
    );
    if (!question) {
...
export async function createPrediction(
  ...
  try {
    ...
    if (
      !Number.isInteger(data.betoption_id) ||
      !Number.isInteger(data.betquestion_id) ||
      !Number.isInteger(data.match_id)
    ) {
      return {
        success: false,
